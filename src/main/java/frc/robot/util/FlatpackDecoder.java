...
public class FlatpackDecoder {
   ...
   * @return DecodedPacket containing schema name and payload buffer, or null if decoding fails
   */
   ...
   *
   * @param rawData Raw binary data from NetworkTables
   * @return Decoded object (float[], Vector2[], or Vector3[]), or null if decoding fails
   */
  ...
  private static Object decodeObjectArray(ByteBuffer buffer, SchemaDescriptor descriptor) {
    buffer.order(ByteOrder.LITTLE_ENDIAN);
    int elementCount =
        readArrayCount(buffer, descriptor.componentCount(), descriptor.name + "_array");
    if (elementCount < 0) {
      return null;
    }
  ...
  private static byte[] fetchManifestBytes() {
    NetworkTableEntry manifestEntry =
        NetworkTableInstance.getDefault().getTable(VISION_TABLE_NAME).getEntry(MANIFEST_ENTRY_NAME);
